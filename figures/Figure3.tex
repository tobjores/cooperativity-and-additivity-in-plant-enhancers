...
\begin{tikzpicture}
	...
	%%% correlation between enhancer strength and TF motif match
	\coordinate[xshift = \columnsep] (cor TF match) at (rbcS-E9 logos dark -| logo rbcS-E9 c.east);
	
	\coordinate (c1) at (cor TF match |- logo rbcS-E9 c.above north);
	\coordinate (c2) at (\textwidth, 0 |- last plot.south);
	
	\distance{c1}{c2}
	
	\begin{hgroupplot}[%
		group position = {anchor = above north west, at = {(cor TF match)}, xshift = \plotylabelwidth + .5\baselineskip},
		axis limits from table = {rawData/PEV_cor_TF_match_axes.tsv},
		enlarge y limits = .05,
		ylabel = Pearson's $r$,
		zero line,
		height = -\ydistance
	]{\xdistance - .5\baselineskip}{2}{enhancer}
		
		\nextgroupplot[
			title = mutagenesis,
			x tick table enhancer = {rawData/PEV_cor_TF_match_mutagenesis_mean.tsv}{enhancer},
		]
			
			\pgfplotstableread{rawData/PEV_cor_TF_match_mutagenesis_mean.tsv}\loadedtable
			\pgfplotstableforeachcolumnelement{enhancer}\of\loadedtable\as\thisenh{
				\pgfplotstablegetelem{\pgfplotstablerow}{mean}\of\loadedtable
				\pgfmathint{\pgfplotstablerow + 1}
				\edef\thisplot{
					\noexpand\draw[very thick, \thisenh] (\pgfplotstablerow + 0.6, \pgfplotsretval) -- (\pgfplotstablerow + 1.4, \pgfplotsretval);
					\noexpand\hjitter[\thisenh!50]{.8}{rawData/PEV_cor_TF_match_mutagenesis_points.tsv}{\pgfmathresult}
				}
				\thisplot
			}
		...
		\nextgroupplot[
			title = scanning,
			x tick table enhancer = {rawData/PEV_cor_TF_match_scanning_mean.tsv}{enhancer},
		]
		
			\pgfplotstableread{rawData/PEV_cor_TF_match_scanning_mean.tsv}\loadedtable
			\pgfplotstableforeachcolumnelement{enhancer}\of\loadedtable\as\thisenh{
				\pgfplotstablegetelem{\pgfplotstablerow}{mean}\of\loadedtable
				\pgfmathint{\pgfplotstablerow + 1}
				\edef\thisplot{
					\noexpand\draw[very thick, \thisenh] (\pgfplotstablerow + 0.6, \pgfplotsretval) -- (\pgfplotstablerow + 1.4, \pgfplotsretval);
					\noexpand\hjitter[\thisenh!50]{.8}{rawData/PEV_cor_TF_match_scanning_points.tsv}{\pgfmathresult}
				}
				\thisplot
			}
