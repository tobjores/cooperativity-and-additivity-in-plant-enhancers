...
\newcommand{\subfiglabel}[2][]{
	\node[anchor = north west, inner sep = 0pt, font = \large\bfseries, #1] at (#2) {\fontspec{Arial}\strut\stepcounter{subfig}\ifsubfigupper\Alph{subfig}\else\alph{subfig}\fi};
}
...
\begin{document}
	...
	\ifmain
		...
		\begin{fig}
			...
			\caption{%
				...
				\subfig{A} All possible single-nucleotide variants of the \enhancer{AB80}, \enhancer{Cab-1}, and \enhancer{rbcS-E9} enhancers were subjected to Plant STARR-seq in \textit{N. benthamiana} leaves. On the morning of the third day after transformation (\timepoint{-8}), the plants were shifted to constant light. Leaves were harvested for RNA extraction starting at mid-day (\timepoint{0}) and in 6 hour intervals (\timepoint{6}, 20, 26, and 32) afterwards.\nextentry
	...
	\ifsupp
		...
		\pagestyle{SuppData}
		\markright{Supplemental Data. Jores et al. (2024). Enhancer cooperativity and additivity. Plant Cell.}
		...
		\begin{sfig}
			...
			\caption{%
				...
				The \enhancer{Cab-1} enhancer was subjected to the same experiment as the \enhancer{AB80} enhancer in \cref{sfig:PEV_cr_heatmap_AB80}.
			}%
		...
		\begin{sfig}
			...
			\caption{%
				...
				The \enhancer{rbcS-E9} enhancer was subjected to the same experiment as the \enhancer{AB80} enhancer in \cref{sfig:PEV_cr_heatmap_AB80}.
			}%
